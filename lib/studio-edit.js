export const PLATFORM_EDIT_KEYS = [
  'imageScale',
  'imageOffsetX',
  'imageOffsetY',
  'imageBrightness',
...
export function pickPlatformEditFields(asset = {}) {
  return PLATFORM_EDIT_KEYS.reduce((acc, key) => {
    if (asset[key] !== undefined) acc[key] = asset[key];
    return acc;
  }, {});
}

export function applyPlatformVariant(draft, platformId) {
  if (!draft?.platforms?.length || !platformId) return draft;
  const variant = draft.platformVariants?.[platformId] || {};
  return {
    ...draft,
    ...variant,
    activePlatformId: platformId,
    cropPreset: variant.cropPreset || draft.cropPreset || 'original',
  };
}

export function commitPlatformVariant(draft, platformId = draft?.activePlatformId) {
  if (!draft?.platforms?.length || !platformId) return draft;
  return {
    ...draft,
    activePlatformId: platformId,
    platformVariants: {
      ...
      [platformId]: pickPlatformEditFields(draft),
    },
  };
}
